...
## Learning Objectives
 ...
 - see additional automation & benefits the multiapps deployment brings
   - linking apps with each other 
   - faster deployment via parallelization
   - zero down time deployment via blue green strategy
   - versioned artefacts consistently deployed across spaces/orgs/landscapes

## Prerequisites

To complete the workspace setup run

    source ${HOME}/summit-hands-on-labs/philadelphia-2019/multiapps/setup-lab.sh
...
IMPORTANT: Execute the tutorial commands in the root of the ~/mtalab directory
...
optional: to see available service offerings: 
...
Let's configure the dashboard with the front end app url via an environment variable:
    ...
    cf set-env fortune-teller-hystrix-dashboard UIURL https://<ui app route>

and restart the app so it will read the new varaible
...
again restart for the change to take effect
...
<a name="deploy"></a>
The **M**ulti **T**arget **A**pplication model provides a powerful abstraction, capable of depicting complicated relationship between different platform entities. You may find detailed information in the SCP online documentation.

Have a look how the fortune teller app is described. Look for the `mtad.yaml` file in the root of the repository:  ~/mtalab/mtad.yaml
...
list all mtas in the current space:

    cf mtas

list details for a speciffic mta: 
...
Instead of `cf deploy` this time run `cf bg-deploy`
    ...
    cf bg-deploy mta_archives/fortune-teller_0.0.2.mtar
...
<a name="undeploy"></a>
You're almost done! To free up resources after the exercise, please remove all parts of the mta with the following:
