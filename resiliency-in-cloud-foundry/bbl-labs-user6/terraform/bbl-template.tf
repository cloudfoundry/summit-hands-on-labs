...
resource "google_compute_instance_group" "router-lb-0" {
  name        = "${var.env_id}-router-lb-0-us-central1-a"
...
resource "google_compute_instance_group" "router-lb-1" {
  name        = "${var.env_id}-router-lb-1-us-central1-b"
  description = "terraform generated instance group that is multi-zone for https loadbalancing"
  zone        = "us-central1-b"
...
resource "google_compute_instance_group" "router-lb-2" {
  name        = "${var.env_id}-router-lb-2-us-central1-c"
  description = "terraform generated instance group that is multi-zone for https loadbalancing"
  zone        = "us-central1-c"
...
resource "google_compute_instance_group" "router-lb-3" {
  name        = "${var.env_id}-router-lb-3-us-central1-f"
  description = "terraform generated instance group that is multi-zone for https loadbalancing"
  zone        = "us-central1-f"
...
resource "google_compute_backend_service" "router-lb-backend-service" {
  ...
  backend {
    group = "${google_compute_instance_group.router-lb-3.self_link}"
  }
...
output "subnet_cidr_4" {
  value = "${cidrsubnet(var.subnet_cidr, 8, 64)}"
}
