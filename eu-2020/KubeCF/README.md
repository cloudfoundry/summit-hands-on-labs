...
* Push an application using `cf push` using `eirini`.
...
- The script will install cf-cli and helm.
...
Running the following command to list the `Cloud Foundry` component pods deployed in `kubecf` namespace.

    kubectl get pods -n kubecf

Your output should display all component pods with `Running` status.
`KubeCF` is already installed for you using `helm`. In case, you are interested to know how it is installed, you can checkout the steps from
